...
select hour(order_time) as hour, count(order_id) as count_order
from orders
...
select round(avg(dquantity), 2) from
(select orders.order_date, sum(order_details.quantity)as dquantity from orders join order_details
